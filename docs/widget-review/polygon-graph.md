### polygon-graph — Manual Widget Review

Principles: require `width`/`height`; minimize nullables; no `.default()`, no `.refine()`.

Scope
- Purpose: draw polygons/polylines by connecting referenced points
...
Schema sketch
```ts
export const PolygonGraphPropsSchema = z.object({
...
}).strict().describe("Creates a coordinate plane for drawing polygons and polylines by connecting named points. Points are defined once and can be reused in multiple polygons. Supports both closed shapes (triangles, quadrilaterals) and open paths. Perfect for coordinate geometry, transformations, and exploring properties of shapes on the coordinate plane.")```

Why this helps
- Removes size fallbacks; inputs are explicit and consistent.
