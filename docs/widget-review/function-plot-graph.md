### function-plot-graph — Manual Widget Review

Principles: require `width`/`height`; minimize nullables; no `.default()`, no `.refine()`.

Scope
- File: `src/lib/widgets/generators/function-plot-graph.ts`
- Purpose: plot polylines (functions) with optional highlighted points

Current pain points
- Nullable size causes fallbacks.
- Optional `points` branch in generator.

Proposed API (no feature loss, fewer nullables)
- Require `width`, `height`.
- Provide `points` as an array (possibly empty) rather than nullable.
...
Why this helps
- Removes fallbacks and null-checks; arrays control presence unambiguously.

Generator guidance
- Always pass `points` array (empty means none to render).
