...
function setupTrigCoordinatePlane(
	data: {
		width: number
		height: number
		xAxis: { label: string; min: number; max: number; tickInterval: number; showGridLines: boolean }
		yAxis: { label: string; min: number; max: number; tickInterval: number; showGridLines: boolean }
	},
	canvas: CanvasImpl
): {
