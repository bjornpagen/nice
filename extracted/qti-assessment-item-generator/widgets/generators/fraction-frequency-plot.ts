...
const TickSchema = z
    .object({
        ...
        frequency: z.number().int().min(0).describe("The number of 'X' marks to stack above this tick, representing its frequency.")
    })
    .strict();

// The main Zod schema for the simplified fractionFrequencyPlot function.
export const FractionFrequencyPlotPropsSchema = z.object({
    ...
    title: z.string().nullable().transform(val => (val === "null" || val === "NULL" || val === "" ? null : val)).describe("Optional title displayed above the plot."),
    ticks: z.array(TickSchema).min(1).describe("An array of the tick marks to display, each with its label and frequency count.")
}).strict().describe("Creates a line plot for fractional data by stacking 'X's above each specified tick based on its frequency. The axis range is automatically determined from the ticks provided.");
 ...
 */
export const generateFractionFrequencyPlot: WidgetGenerator<typeof FractionFrequencyPlotPropsSchema> = async (props) => {
    ...
    const drawFractionLabel = (x: number, y: number, label: FractionLabel) => {
        const fontStyles = { anchor: "middle" as const, dominantBaseline: "middle" as const, fontPx: 14 };
        ...
        if (label.type === "fraction") {
            ...
            canvas.drawText({ x, y: y - FRACTION_OFFSET, text: numText, ...fontStyles });
            canvas.drawLine(x - fracWidth / 2, y - 1, x + fracWidth / 2, y - 1, { stroke: theme.colors.axisLabel, strokeWidth: 1.5 });
            canvas.drawText({ x, y: y + FRACTION_OFFSET, text: denText, ...fontStyles });
            return;
        }
        // mixed
        ...
        canvas.drawText({ x: fracX, y: y - FRACTION_OFFSET, text: numText, ...fontStyles });
        canvas.drawLine(fracX - fracWidth / 2, y - 1, fracX + fracWidth / 2, y - 1, { stroke: theme.colors.axisLabel, strokeWidth: 1.5 });
        canvas.drawText({ x: fracX, y: y + FRACTION_OFFSET, text: denText, ...fontStyles });
    };
