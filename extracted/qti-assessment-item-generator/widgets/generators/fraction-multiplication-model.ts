...
const createBaseFractionModelSchema = () => z.object({
    ...
    shadeColor: z.string().regex(CSS_COLOR_PATTERN, "invalid css color").describe("The CSS hex color for the shaded parts (e.g., '#4472C4')."),
});
...
const createMiddleTermSchema = () => z.discriminatedUnion("type", [
    z.object({
...
export const FractionMultiplicationModelPropsSchema = z.object({
    type: z.literal("fractionMultiplicationModel").describe("Identifies this as a widget for showing a fraction multiplication equation using visual models."),
    ...
    rightOperand: createBaseFractionModelSchema().describe("The fraction model on the right side of the equation (the product)."),
}).strict();
...
export const generateFractionMultiplicationModel: WidgetGenerator<typeof FractionMultiplicationModelPropsSchema> = async (props) => {
    ...
    canvas.drawText({ x: op1X, y: verticalCenter, text: "×", anchor: "middle", dominantBaseline: "middle", fontPx: operatorFontSize });
    currentX += operatorWidth;

    // --- Draw Middle Term ---
    const middleCx = currentX + middleTermWidth / 2;
    if (middleTerm.type === "number") {
        canvas.drawText({ x: middleCx, y: verticalCenter, text: String(middleTerm.value), anchor: "middle", dominantBaseline: "middle", fontPx: operatorFontSize });
    } else {
    ...
    canvas.drawText({ x: op2X, y: verticalCenter, text: "=", anchor: "middle", dominantBaseline: "middle", fontPx: operatorFontSize });
    currentX += operatorWidth;
