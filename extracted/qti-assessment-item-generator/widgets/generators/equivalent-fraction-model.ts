...
export const EquivalentFractionModelPropsSchema = z.object({
    type: z.literal("equivalentFractionModel").describe("Identifies this as a widget for comparing equivalent fractions (tenths and hundredths)."),
    width: z.number().positive().describe("Total width of the SVG in pixels. Should accommodate two squares and spacing."),
    height: z.number().positive().describe("Total height of the SVG in pixels."),
    numerator: z.number().int().min(0).max(10).describe("The numerator for the tenths fraction (e.g., for 3/10, this value is 3). Must be between 0 and 10."),
    tenthsColor: z.string().regex(CSS_COLOR_PATTERN, "invalid css color").describe("CSS color for the shaded tenths bars."),
    hundredthsColor: z.string().regex(CSS_COLOR_PATTERN, "invalid css color").describe("CSS color for the shaded hundredths squares."),
}).strict().describe("Creates a visual comparison of a fraction in tenths and its equivalent in hundredths using two partitioned squares. The left square shows shaded vertical bars for tenths, and the right square shows a shaded 10x10 grid for hundredths.");
 ...
 */
export const generateEquivalentFractionModel: WidgetGenerator<typeof EquivalentFractionModelPropsSchema> = async (props) => {
    ...
    for (let i = 1; i < 10; i++) {
        ...
        canvas.drawLine(x, yPos, x, yPos + boxSize, { stroke: theme.colors.black, strokeWidth: theme.stroke.width.thin });
        canvas.drawLine(rightBoxX, y, rightBoxX + boxSize, y, { stroke: theme.colors.black, strokeWidth: theme.stroke.width.thin });
    }
    // Outer border
