...
const mockProcessResponse = mock((_identifier: string, input: { responseIdentifier: string; value: unknown }) => {
	...
	if (input.responseIdentifier === "RESPONSE") {
		...
		return Promise.reject(
			errors.new('Response declaration with identifier "RESPONSE" does not exist in the assessment item')
		)
	}
	return Promise.reject(errors.new(`unexpected identifier: ${input.responseIdentifier}`))
})
