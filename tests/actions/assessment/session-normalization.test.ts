...
describe("Session Result Normalization", () => {
	test("should only count the first boolean attempt for a question, ignoring subsequent correct answers", async () => {
		mockCheckExistingProficiency.mockImplementation((_u, _a) => Promise.resolve(false))
		await finalizeAssessment({
			...defaultOptions,
			sessionResults: [
				// User gets q1 wrong, then right. It should count as wrong.
				{ qtiItemId: "q1", isCorrect: false, isReported: false },
				{ qtiItemId: "q1", isCorrect: true, isReported: false },
				// q2 is correct on the first try.
				{ qtiItemId: "q2", isCorrect: true, isReported: false }
			]
		})

		const metadata = gradebookSpy.mock.calls[0]?.[0]?.metadata
		expect(metadata?.totalQuestions).toBe(2)
		expect(metadata?.correctQuestions).toBe(1) // Only q2 is correct
		expect(metadata?.accuracy).toBe(50)
	})

	test("should exclude questions that were reported at any point", async () => {
		mockCheckExistingProficiency.mockImplementation((_u, _a) => Promise.resolve(false))
		await finalizeAssessment({
			...defaultOptions,
			sessionResults: [
				{ qtiItemId: "q1", isCorrect: true, isReported: false },
				// q2 is answered correctly but then reported. It should be excluded entirely.
				{ qtiItemId: "q2", isCorrect: true, isReported: false },
				{ qtiItemId: "q2", isCorrect: null, isReported: true }
			]
		})

		const metadata = gradebookSpy.mock.calls[0]?.[0]?.metadata
		expect(metadata?.totalQuestions).toBe(1) // Only q1 is counted
