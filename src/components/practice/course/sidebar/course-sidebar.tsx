...
import { CourseSidebarCourseMaterialItem } from "./course-sidebar-course-material-item"
...
export function CourseSidebar({
	...
	if (materials.length === 0) {
		throw errors.new("no content found for course")
	}

	const [index, setIndex] = React.useState<number>(
		_.clamp(
			_.findIndex(materials, (material) => material.path === pathname),
			0,
			materials.length - 1
		)
	)
		...
		<div id="course-sidebar" className={cn("bg-gray-100 border-r border-gray-200 flex flex-col h-full p-4", className)}>
			<div className="px-6 pb-4 flex-1 overflow-hidden bg-white rounded-md shadow-md">
				<ScrollArea className="h-full">
					...
					<div>
						<CourseSidebarCourseMaterialItem index={index} materials={materials} pathname={pathname} />
					</div>
