...
import {
  getCurrentUserSourcedId,
  isUserAuthorizedForQuestion,
...
async function getDeterministicQuestionList(
  ...
  const { assessmentTest, resolvedQuestions } = await fetchAndResolveQuestions(
    assessmentResourceSourcedId,
  );

  const questions = applyQtiSelectionAndOrdering(
    assessmentTest,
    resolvedQuestions,
    {
...
export async function processQuestionResponse(
  ...
  if (itemResult.error) {
    logger.error("failed to get qti assessment item for response processing", {
  ...
  if (
    ...
    const responseEntries = originalEntries.filter(([id]) =>
      allowedIds.has(id),
    );
    ...
    logger.info("processing multi-input question", {
      ...
      ignoredIdentifiers: originalEntries
    ...
    if (responseEntries.length === 0) {
      logger.error(
        "no declared response identifiers present in selected response",
        {
          qtiItemId,
    ...
    const qtiResult = await errors.try(
      qti.processResponses(qtiItemId, responsesMap),
    );

    if (qtiResult.error) {
      logger.error("qti batched response processing failed", {
        error: qtiResult.error,
        qtiItemId,
        responseCount: responseEntries.length,
      });
      throw errors.wrap(qtiResult.error, "qti batched response processing");
    }

    // The QTI API returns score: 1 for all correct, score: 0 for any incorrect
    const isAllCorrect = qtiResult.data.score > 0;
 ...
 */
export async function getOrCreateAssessmentState(
  ...
  const existingState = await getAssessmentState(
    onerosterUserSourcedId,
    onerosterResourceSourcedId,
    attemptNumber,
  );
...
export async function submitAnswer(
  ...
  if (!state) {
    logger.error("assessment state not found", {
      onerosterUserSourcedId,
      onerosterResourceSourcedId,
      attemptNumber,
    });
  ...
  if (itemResult.error) {
    logger.error("failed to get qti assessment item for submit", {
      error: itemResult.error,
      questionId,
    });
    throw errors.wrap(itemResult.error, "qti get assessment item");
  ...
  if (
    ...
    // Send all responses in a single batched request
    const qtiResult = await errors.try(
      qti.processResponses(questionId, responsesMap),
    );

    if (qtiResult.error) {
      logger.error("qti batched response processing failed for submit", {
        error: qtiResult.error,
        qtiItemId: questionId,
        responseCount: responseEntries.length,
      });
...
export async function skipQuestion(
  ...
  if (state.currentQuestionIndex < questionIndex) {
    logger.error("future skip received", {
      currentIndex: state.currentQuestionIndex,
      submittedIndex: questionIndex,
    });
  ...
  const skipState: AssessmentQuestionState = {
    isCorrect: false,
    response: null,
    isReported: false,
  };
...
export async function reportQuestion(
  ...
  const currentState = await getAssessmentState(
    onerosterUserSourcedId,
    onerosterResourceSourcedId,
    attemptNumber,
  );
  ...
  if (updateResult.error) {
    ...
    throw errors.wrap(
      updateResult.error,
      "update redis state for reported question",
    );
  }
  ...
  const flagResult = await errors.try(
    flagQuestionAsReported(questionId, report),
  );
  if (flagResult.error) {
    logger.error(
      "non-critical: failed to flag question in external service after state update",
      {
  ...
  const newState = await getAssessmentState(
    onerosterUserSourcedId,
    onerosterResourceSourcedId,
    attemptNumber,
  );
...
export async function checkExistingProficiency(
  ...
  const strictLineItemId = getAssessmentLineItemId(
    onerosterAssessmentSourcedId,
  );
  ...
  const latestResult = findLatestInteractiveAttempt(
    resultsResult.data,
    onerosterUserSourcedId,
    strictLineItemId,
  );
  ...
  const normalizedScore =
    latestResult.score <= 1.1 ? latestResult.score * 100 : latestResult.score;
...
export async function accumulateAssessmentTime(
  ...
  if (!userId) {
    logger.debug(
      "assessment time accumulate: user not authenticated, skipping",
    );
    return;
  }
  ...
  const { getAssessmentTimeState, setAssessmentTimeState } =
    await import("@/lib/assessment-time-cache");

  const now = new Date();
  const currentState = (await getAssessmentTimeState(
    onerosterUserSourcedId,
    onerosterResourceSourcedId,
    attemptNumber,
  )) ?? {
  ...
  if (currentState.lastServerSyncAt) {
    const sinceMs =
      now.getTime() - new Date(currentState.lastServerSyncAt).getTime();
  ...
  const newState = {
    cumulativeActiveSeconds:
      currentState.cumulativeActiveSeconds + effectiveDelta,
    lastServerSyncAt: now.toISOString(),
  };

  await setAssessmentTimeState(
    onerosterUserSourcedId,
    onerosterResourceSourcedId,
    attemptNumber,
    newState,
...
export async function finalizeAssessment(options: {
  ...
  if (!clerkUserId) {
    logger.error("finalize assessment: user not authenticated", {
      correlationId,
    });
    throw errors.new("user not authenticated");
  }
  ...
  const performFinalization = async () => {
    ...
    if (state.finalSummary) {
      logger.info(
        "finalizeAssessment called on an already finalized state, returning stored summary.",
        {
          correlationId,
        },
      );
    ...
    const sessionResults = Object.entries(state.questions).map(
      ([indexStr, qState]) => {
        ...
        if (!questionId) {
          logger.error(
            "critical state inconsistency: question ID not found at expected index during finalization",
            {
              onerosterUserSourcedId,
              onerosterResourceSourcedId: options.onerosterResourceSourcedId,
              attemptNumber,
          ...
          throw errors.new(
            "critical state inconsistency: could not map question state to a question ID",
          );
        }
    ...
    const scorableResults = sessionResults.filter(
    ...
    const avgSecondsPerQuestion =
      durationInSeconds && totalQuestions > 0
        ? durationInSeconds / totalQuestions
        : undefined;
    ...
    if (pathParts.length < 3 || !pathParts[1] || !pathParts[2]) {
      logger.error("invalid assessment path structure", {
    ...
    if (saveResult.error) {
      ...
      throw errors.wrap(
        saveResult.error,
        "failed to save final assessment result",
      );
    }

    const extractXpInfo = ():
      | { finalXp: number; penaltyApplied: boolean; reason: string }
      | undefined => {
      if (
        !saveResult.data ||
        typeof saveResult.data !== "object" ||
        !("xp" in saveResult.data)
      ) {
        return undefined;
      }
    ...
    if (options.contentType === "CourseChallenge") {
      ...
      if (sendResult.error) {
        ...
        logger.info(
          "emitted course-challenge.completed event for server-side progression",
          {
            courseSourcedId: options.onerosterCourseSourcedId,
            score,
            correlationId,
  ...
  if (finalizationResult.error) {
    ...
    logger.error("assessment finalization failed unexpectedly", {
      correlationId,
 ...
 */
export async function flagQuestionAsReported(
  questionId: string,
  report: string,
  ...
  const isAuthorized = await isUserAuthorizedForQuestion(
    userSourcedId,
    questionId,
  );
  if (!isAuthorized) {
    logger.warn("unauthorized attempt to report question", {
      clerkUserId,
      userSourcedId,
      questionId,
    });
  ...
  logger.info("flagging question as reported", {
    clerkUserId,
    questionId,
    report,
  });
  ...
  if (flagResult.error) {
    logger.error("failed to flag question in qti api", {
      clerkUserId,
      questionId,
      error: flagResult.error,
    });
 ...
 */
export async function startNewAssessmentAttempt(
  onerosterResourceSourcedId: string,
): Promise<{ success: boolean }> {
  const { userId } = await auth();
  if (!userId) {
