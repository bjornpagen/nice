...
type LessonPlanTreeComponentResource = z.infer<typeof LessonPlanTreeComponentResourceSchema>

const LessonPlanTreeComponentSchema: z.ZodType<any> = z.lazy(() =>
	z
		.object({
			id: z.string(),
			sourcedId: z.string(),
...
const LessonPlanTreeSchema = z.object({
	lessonPlan: z.object({
		id: z.string().optional(),
		course: z.object({}).passthrough(),
		subComponents: z.array(LessonPlanTreeComponentSchema)
	})
})

const AssessmentResultLiteSchema = z.object({
    tenantId: z.string(),
    ...
    scoreDate: z.string().optional(),       // sometimes present
    scoreScaleSourcedId: z.string().nullable().optional(),
    scorePercentile: z.number().nullable().optional(),
    scoreStatus: z.string(),                // e.g. "fully graded"
    comment: z.string().nullable().optional(),
    learningObjectiveSet: z.any().nullable().optional(),
    inProgress: z.string().nullable().optional(),  // "true"/"false"
...
const ComponentLineItemSchema = z.object({
    ...
    results: z.array(AssessmentResultLiteSchema),
  }).passthrough()
...
const GetCourseProgressResultSchema = z.object({
	lineItems: z.array(z.discriminatedUnion("type", [ComponentLineItemSchema, ResourceLineItemSchema]))
})
...
export type LessonPlanTreeResult = z.infer<typeof LessonPlanTreeSchema>
export type CourseProgressResult = z.infer<typeof GetCourseProgressResultSchema>
...
export class Client {
	...
	// NEW: Get Lesson Plan tree for a course and student
	async getLessonPlanTreeForStudent(courseId: string, userId: string): Promise<LessonPlanTreeResult> {
		const endpoint = `/powerpath/lessonPlans/${encodeURIComponent(courseId)}/${encodeURIComponent(userId)}`
		return this.#request(endpoint, { method: "GET" }, LessonPlanTreeSchema)
	}

	// NEW: Get Course Progress for a course and student (optional lessonId filter)
	async getCourseProgress(courseId: string, studentId: string, lessonId?: string): Promise<CourseProgressResult> {
		let endpoint = `/powerpath/lessonPlans/getCourseProgress/${encodeURIComponent(courseId)}/student/${encodeURIComponent(
			studentId
		)}`
		if (lessonId) {
			endpoint += `?lessonId=${encodeURIComponent(lessonId)}`
		}
		return this.#request(endpoint, { method: "GET" }, GetCourseProgressResultSchema)
	}
