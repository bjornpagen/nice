...
import { db } from "@/db"
import { niceArticles, niceCourses, niceExercises, niceVideos } from "@/db/schemas"
...
interface LinkInfo {
	...
	type: "a" | "e" | "v"
	slug: string
	isKhanAcademy?: boolean
}

/**
 * Resolve relative hrefs and Khan Academy links in QTI stimulus XML to absolute canonical URLs using DB paths.
 *
 * Rules:
 * - Rewrites anchors with relative hrefs (start with "/")
 * - Also rewrites khanacademy.org links to nice.academy URLs
 ...
 * - If any link cannot be resolved, the function logs and throws to prevent silent fallbacks
 */
export async function resolveRelativeLinksToCanonicalDomain(xml: string, slog: logger.Logger): Promise<string> {
	...
	for (const m of matches) {
		...
		// Handle Khan Academy URLs
		if (url.includes("khanacademy.org")) {
			// Parse Khan Academy URL to extract type and slug
			const urlParts = url.split("/").filter((p) => p !== "")
			if (urlParts.length < 2) continue
			const slug = urlParts[urlParts.length - 1] ?? ""
			const typeSegment = urlParts[urlParts.length - 2] ?? ""
			if (typeSegment !== "a" && typeSegment !== "e" && typeSegment !== "v") continue
			if (slug === "") continue
			candidates.push({ full: url, type: typeSegment, slug, isKhanAcademy: true })
			continue
		}
		...
		if (url.startsWith("/")) {
			...
			if (typeSegment !== "a" && typeSegment !== "e" && typeSegment !== "v") continue
			if (slug === "") continue
			candidates.push({ full: url, type: typeSegment, slug, isKhanAcademy: false })
		}
	}

	if (candidates.length === 0) return xml

	slog.debug("resolving relative and Khan Academy links to canonical domain", {
	...
	if (allowedCoursePrefixes.size > 0) {
		...
		if (urlsToRemove.size > 0) {
			...
			for (const urlToStrip of urlsToRemove) {
				// Handle both relative and Khan Academy URLs in anchor stripping
				const anchorRegex = new RegExp(`<a\\b[^>]*href="${escapeRegExp(urlToStrip)}"[^>]*>([\\s\\S]*?)<\\/a>`, "g")
	...
	for (const m of remainingMatches) {
		...
		if (url.includes("khanacademy.org")) {
			...
			if (typeSegment !== "a" && typeSegment !== "e" && typeSegment !== "v") continue
			if (slug === "") continue
			remainingCandidates.push({ full: url, type: typeSegment, slug, isKhanAcademy: true })
			continue
		}
		...
		if (url.startsWith("/")) {
			...
			if (typeSegment !== "a" && typeSegment !== "e" && typeSegment !== "v") continue
			if (slug === "") continue
			remainingCandidates.push({ full: url, type: typeSegment, slug, isKhanAcademy: false })
		}
	}

	// Separate candidates by type and check resolvability
	const unresolvableKhanLinks: LinkInfo[] = []
	const unresolvableRelativeLinks: LinkInfo[] = []
	...
	for (const c of remainingCandidates) {
		...
		if (!found) {
			if (c.isKhanAcademy) {
				unresolvableKhanLinks.push(c)
			} else {
				unresolvableRelativeLinks.push(c)
			}
		}
	}

	// Strip unresolvable Khan Academy links (don't fail, just remove them)
	if (unresolvableKhanLinks.length > 0) {
		slog.warn("stripping unresolvable khan academy links", {
			count: unresolvableKhanLinks.length,
			links: unresolvableKhanLinks.map((l) => ({ url: l.full, type: l.type, slug: l.slug }))
		})

		const escapeRegExp = (s: string) => s.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")
		for (const link of unresolvableKhanLinks) {
			const anchorRegex = new RegExp(`<a\\b[^>]*href="${escapeRegExp(link.full)}"[^>]*>([\\s\\S]*?)<\\/a>`, "g")
			workingXml = workingXml.replace(anchorRegex, (_m, inner: string) => inner)
		}
	}

	// Fail for unresolvable relative links (these are internal and must exist)
	if (unresolvableRelativeLinks.length > 0) {
		for (const link of unresolvableRelativeLinks) {
			slog.error("relative link could not be resolved by type+slug", {
				url: link.full,
				type: link.type,
				slug: link.slug
			})
		}
		slog.error("failing due to unresolvable relative links", { count: unresolvableRelativeLinks.length })
		throw errors.new("unresolvable relative links")
	}
	...
	const rewrittenXml = workingXml.replace(/href="([^"#?]*)"/g, (_match, url: string) => {
		...
		if (url.includes("khanacademy.org")) {
			...
			if (urlParts.length < 2) return `href="${url}"`
			const slug = urlParts[urlParts.length - 1] ?? ""
			const typeSegment = urlParts[urlParts.length - 2] ?? ""
			let canonicalPath = ""
	...
	slog.debug("resolved relative and Khan Academy links to canonical domain", {
		totalCandidates: remainingCandidates.length,
		strippedKhanLinks: unresolvableKhanLinks.length,
		resolvedLinks: remainingCandidates.length - unresolvableKhanLinks.length - unresolvableRelativeLinks.length
	})
