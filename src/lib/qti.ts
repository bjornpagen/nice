...
export const ErrQtiUnprocessable = errors.new(
  "qti request unprocessable/invalid",
);
export const ErrQtiConflict = errors.new("qti resource conflict");
export const ErrQtiInternalServerError = errors.new(
  "qti internal server error",
);
...
export type SearchAssessmentItemsResponse = z.infer<
 ...
 */
const CreateAssessmentTestInputSchema = z
  .string()
  .min(1, { message: "XML content cannot be empty" });
export type CreateAssessmentTestInput = z.infer<
  typeof CreateAssessmentTestInputSchema
>;

/**
 * Input for updating an Assessment Test.
 */
export type UpdateAssessmentTestInput = z.infer<
  typeof CreateAssessmentTestInputSchema
>;
...
export type SearchAssessmentTestsInput = z.infer<
  typeof SearchAssessmentTestsInputSchema
>;
...
export type SearchAssessmentTestsResponse = z.infer<
  typeof SearchAssessmentTestsResponseSchema
>;
...
export class Client {
  ...
  async #getAccessToken(): Promise<string> {
    ...
    if (!result.data.ok) {
      ...
      logger.error("qti auth: token request rejected", {
        status: result.data.status,
        body: errorBody,
      });
      throw errors.new(
        `qti token request failed with status ${result.data.status}`,
      );
    }

    const jsonResult = await errors.try(result.data.json());
    if (jsonResult.error) {
      logger.error("qti auth: failed to parse token response", {
    ...
    if (!validation.success) {
      logger.error("qti auth: invalid token response schema", {
  ...
  async #request<T>(
    ...
    if (fetchResult.error) {
      logger.error("qti api request failed", {
    ...
    if (!response.ok) {
      ...
      logger.error("qti api returned non-ok status", {
        q: response.status,
        body: errorBody,
        endpoint,
      });
      ...
      if (isJwtExpired) {
        logger.info("qti auth: jwt expired, attempting to refresh token", {
          endpoint,
        });
        ...
        const retryResult = await errors.try(
          fetch(url, { ...options, headers: retryHeaders }),
        );
        if (retryResult.error) {
          logger.error("qti api request failed after token refresh", {
            error: retryResult.error,
            endpoint,
          });
          throw errors.wrap(
            retryResult.error,
            "qti api request after token refresh",
          );
        }
        ...
        if (!retryResponse.ok) {
          ...
          throw errors.wrap(
            ErrJWTExpired,
            `qti jwt refresh failed: status ${retryResponse.status} on ${endpoint}`,
          );
        }
        ...
        if (retryJsonResult.error) {
          logger.error(
            "qti api: failed to parse json response after token refresh",
            {
          ...
          throw errors.wrap(
            ...
            "qti api response parsing after token refresh",
          );
        }
        logger.debug("qti api: raw response after token refresh", {
          endpoint,
          response: retryJsonResult.data,
        });
        ...
        if (!retryValidation.success) {
          ...
          throw errors.wrap(
            ...
            "qti api response validation after token refresh",
          );
        }
      ...
      if (response.status === 404) {
        logger.error("qti api not found", {
          endpoint,
          status: response.status,
        });
        throw errors.wrap(
          ErrQtiNotFound,
          `qti api error: status 404 on ${endpoint}`,
        );
      }
      if (response.status === 422) {
        logger.error("qti api unprocessable", {
          endpoint,
          status: response.status,
          errorBody,
        });
        throw errors.wrap(
          ErrQtiUnprocessable,
          `qti api error: status 422 on ${endpoint}: ${errorBody}`,
        );
      }
      if (response.status === 500) {
        logger.error("qti api internal server error", {
          endpoint,
          status: response.status,
          errorBody,
        });
        throw errors.wrap(
          ErrQtiInternalServerError,
          `qti api error: status 500 on ${endpoint}: ${errorBody}`,
        );
      }
      if (response.status === 409) {
        ...
        throw errors.wrap(
          ErrQtiConflict,
          `qti api error: status 409 on ${endpoint}`,
        );
      }

      logger.error("qti api error", { endpoint, status: response.status });
      throw errors.new(
        `qti api error: status ${response.status} on ${endpoint}`,
      );
    }
    ...
    if (jsonResult.error) {
      logger.error("qti api: failed to parse json response", {
        error: jsonResult.error,
        endpoint,
      });
    ...
    logger.debug("qti api: raw response", {
      endpoint,
        ...
        if (Object.hasOwn(jsonResult.data, key)) {
          ...
          if (maybeWrapped !== undefined) {
            ...
            if (unwrappedValidation.success) {
              logger.info("qti api: unwrapped response envelope", {
                endpoint,
                envelope: key,
              });
  ...
  async #createEntity<T extends z.ZodType, U, R>(
    ...
    if (!validationResult.success) {
      ...
      throw errors.wrap(
        ...
        `invalid input for ${entityName}`,
      );
    }

    const payload = transformPayload
      ? transformPayload(validationResult.data)
      : { [entityName]: validationResult.data };
  ...
  async createStimulus(input: CreateStimulusInput): Promise<Stimulus> {
    logger.info("qti client: creating stimulus", {
   ...
   */
  async searchStimuli(
    input: SearchStimuliInput,
  ): Promise<SearchStimuliResponse> {
    logger.info("qti client: searching stimuli", { input });
    const validation = SearchStimuliInputSchema.safeParse(input);
    if (!validation.success) {
      logger.error("qti client: invalid input for searchStimuli", {
        error: validation.error,
      });
  ...
  async getAssessmentItem(identifier: string): Promise<AssessmentItem> {
    ...
    if (!identifier) {
      logger.error("identifier is required for getAssessmentItem", {
        identifier,
      });
      throw errors.new(
        "qti client: identifier is required for getAssessmentItem",
      );
    }
    return this.#request(
  ...
  async updateAssessmentItem(input: UpdateItemInput): Promise<AssessmentItem> {
    logger.info("qti client: updating assessment item", {
      identifier: input.identifier,
    });
    const validation = UpdateItemInputSchema.safeParse(input);
    if (!validation.success) {
      logger.error("qti client: invalid input for updateAssessmentItem", {
        error: validation.error,
      });
      throw errors.wrap(
        validation.error,
        "updateAssessmentItem input validation",
      );
    }
  ...
  async deleteAssessmentItem(identifier: string): Promise<void> {
    ...
    if (!identifier) {
      logger.error("identifier is required for deleteAssessmentItem", {
        identifier,
      });
      throw errors.new(
        "qti client: identifier is required for deleteAssessmentItem",
      );
    }
  ...
  ): Promise<SearchAssessmentItemsResponse> {
    logger.info("qti client: searching assessment items", { input });
    const validation = SearchItemsInputSchema.safeParse(input);
    if (!validation.success) {
      logger.error("qti client: invalid input for searchAssessmentItems", {
        error: validation.error,
      });
      throw errors.wrap(
        validation.error,
        "searchAssessmentItems input validation",
      );
    }
  ...
  async createAssessmentTest(
    xml: CreateAssessmentTestInput,
  ): Promise<AssessmentTest> {
    const validationResult = CreateAssessmentTestInputSchema.safeParse(xml);
    if (!validationResult.success) {
      logger.error("qti client: invalid input for createAssessmentTest", {
        error: validationResult.error,
      });
      throw errors.wrap(
        validationResult.error,
        "createAssessmentTest input validation",
      );
    }
  ...
  async getAssessmentTest(identifier: string): Promise<AssessmentTest> {
    ...
    if (!identifier) {
      logger.error("identifier is required for getAssessmentTest", {
        identifier,
      });
      throw errors.new(
        "qti client: identifier is required for getAssessmentTest",
      );
    }
    return this.#request(
  ...
  async updateAssessmentTest(
    identifier: string,
    xml: UpdateAssessmentTestInput,
  ): Promise<AssessmentTest> {
    const validationResult = CreateAssessmentTestInputSchema.safeParse(xml);
    if (!validationResult.success) {
      logger.error("qti client: invalid input for updateAssessmentTest", {
        error: validationResult.error,
      });
      throw errors.wrap(
        validationResult.error,
        "updateAssessmentTest input validation",
      );
    }
    logger.info("qti client: updating assessment test from XML", {
      identifier,
    });
    return this.#request(
      `/assessment-tests/${identifier}`,
  ...
  async deleteAssessmentTest(identifier: string): Promise<void> {
    ...
    if (!identifier) {
      logger.error("identifier is required for deleteAssessmentTest", {
        identifier,
      });
      throw errors.new(
        "qti client: identifier is required for deleteAssessmentTest",
      );
    }
    await this.#request(
      `/assessment-tests/${identifier}`,
  ...
  async searchAssessmentTests(
    input: SearchAssessmentTestsInput,
  ): Promise<SearchAssessmentTestsResponse> {
    logger.info("qti client: searching assessment tests", { input });
    const validation = SearchAssessmentTestsInputSchema.safeParse(input);
    if (!validation.success) {
      logger.error("qti client: invalid input for searchAssessmentTests", {
        error: validation.error,
      });
      throw errors.wrap(
        validation.error,
        "searchAssessmentTests input validation",
      );
    }
    const params = new URLSearchParams();
    for (const [key, value] of Object.entries(validation.data)) {
    ...
    return this.#request(
      endpoint,
      { method: "GET" },
      SearchAssessmentTestsResponseSchema,
    );
  }
  ...
  async processResponse(
    ...
    if (!validation.success) {
      logger.error("processResponse input validation failed", {
        identifier,
        input,
        error: validation.error,
      });
    ...
    if (!apiValidation.success) {
      logger.error("processResponse api request validation failed", {
        error: apiValidation.error,
        identifier,
      });
      throw errors.wrap(
        apiValidation.error,
        "processResponse api request validation",
      );
    }
  ...
  async processResponses(
    ...
    const apiValidation =
      ProcessResponsesBatchedApiRequestSchema.safeParse(payload);
    if (!apiValidation.success) {
      logger.error("processResponses api request validation failed", {
        error: apiValidation.error,
        identifier,
      });
      throw errors.wrap(
        apiValidation.error,
        "processResponses api request validation",
      );
    }

    logger.debug("qti client: prepared batched api request", {
      identifier,
      responses,
      payload: apiValidation.data,
    });
  ...
  async validateXml(input: ValidateXmlInput): Promise<ValidateXmlResponse> {
    logger.info("qti client: validating xml", {
      schema: input.schema,
