...
export const CoordinatePlanePropsSchema = z
	...
	.describe(
		'This is a powerful and versatile template designed to generate a complete, standards-compliant, two-dimensional Cartesian coordinate plane as an SVG graphic. The resulting SVG is embedded within an HTML <div>, ensuring it is scalable, accessible, and ready for integration into any QTI assessment item. The generator constructs a full coordinate system with the following highly configurable features: Axes and Grid: It renders X and Y axes, which can be customized with specific minimum and maximum values, as well as a defined interval for tick marks. The axes can be labeled with standard "x" and "y" or with descriptive titles for word problems (e.g., "Time (seconds)", "Temperature (°C)"). Optional grid lines can be displayed for one or both axes to aid in reading coordinates. Quadrant Labels: The template can automatically render the standard quadrant labels ("I", "II", "III", "IV") in their correct positions. This feature can be turned off for a cleaner look when not needed. Point Plotting: The core function is to plot one or more points on the plane. Each point is defined by its (x, y) coordinates and can be given a unique text label (e.g., "A", "B", "(m, n)"). Points are rendered as clearly visible circles. Polygon Drawing: The template can connect a specified sequence of plotted points to form a closed, filled polygon. This is ideal for geometry problems involving triangles, rectangles, parallelograms, or other shapes on the coordinate plane. The fill color and opacity of the polygon can be customized to make it stand out. This single template is sufficient to generate all the diagrams found in the provided examples, from simple quadrant identification questions to complex problems involving the area and perimeter of polygons defined by coordinates.'
	)
