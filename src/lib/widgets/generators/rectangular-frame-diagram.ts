...
const Diagonal = z.object({ 
  ...
  label: z.string().nullable().transform((val) => (val === "null" || val === "NULL" || val === "" ? null : val)).describe("Text label for the diagonal's length (e.g., '15 cm', 'd = 13', '√50', null). Null means no label. Positioned at midpoint."), 
...
export const RectangularFrameDiagramPropsSchema = z.object({
  ...
  shadedFace: z.string().nullable().transform((val) => (val === "null" || val === "NULL" || val === "" ? null : val)).describe("Face identifier to shade/highlight: 'top_face', 'side_face', 'front_face', 'bottom_face', or null. Null means no shading."),
...
export const generateRectangularFrameDiagram: WidgetGenerator<typeof RectangularFrameDiagramPropsSchema> = (data) => {
	const { width, height, outerLength, outerWidth, outerHeight, thickness, labels, diagonals, shadedFace, showHiddenEdges } = data
	...
	if (thickness <= 0 || thickness >= Math.min(outerLength, outerWidth, outerHeight)) {
		throw errors.wrap(ErrInvalidDimensions, `thickness: ${thickness} must be positive and less than all outer dimensions`)
	}
