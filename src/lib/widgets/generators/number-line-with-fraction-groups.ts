...
const Tick = z.object({ 
  ...
  label: z.string().nullable().transform((val) => (val === "null" || val === "NULL" || val === "" ? null : val)).describe("Text label for this tick (e.g., '0', '1/2', '1', '1 1/2', null). Null shows tick without label. Can show fractions or mixed numbers."), 
...
const Segment = z.object({ 
  ...
  ).describe("CSS fill color for this segment's bar (e.g., '#FFE5B4' for peach, 'lightblue', 'rgba(255,0,0,0.3)'). Different colors distinguish groups."), 
  label: z.string().nullable().transform((val) => (val === "null" || val === "NULL" || val === "" ? null : val)).describe("Text label for this segment (e.g., '1/2', 'Group A', '0.5', null). Positioned above the segment. Null shows no label. Can indicate the fraction size or group name.") 
}).strict()
