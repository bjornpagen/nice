...
export const QuantityFractionalDiagramPropsSchema = z.object({
    type: z.literal("quantityFractionalDiagram").describe("Identifies this as a diagram for displaying fractional quantities (simple, mixed, or improper) using partitioned shapes."),
    ...
    shape: z.discriminatedUnion("type", [
        ...
        z.object({
            ...
            sides: z.number().int().gte(3).describe("Number of sides for the regular polygon (e.g., 3 for triangle, 8 for octagon)."),
    ...
    numerator: z.number().int().min(0).describe("The total numerator of the quantity (e.g., for 2 3/4, the total numerator is 11)."),
    denominator: z.number().int().positive().describe("The denominator that defines the partitioning of each shape (e.g., for 2 3/4, this is 4)."),
    shadeColor: z.string().regex(CSS_COLOR_PATTERN, "invalid css color").describe("The CSS hex color for the shaded parts."),
...
function drawSingleModel(
    ...
    switch (shape.type) {
        case "rectangle": {
            ...
            for (let r = 0; r <= rows; r++) canvas.drawLine(rectX, rectY + r * cellHeight, rectX + size, rectY + r * cellHeight, { stroke: theme.colors.black, strokeWidth: r === 0 || r === rows ? theme.stroke.width.thick : theme.stroke.width.thin });
            for (let c = 0; c <= columns; c++) canvas.drawLine(rectX + c * cellWidth, rectY, rectX + c * cellWidth, rectY + size, { stroke: theme.colors.black, strokeWidth: c === 0 || c === columns ? theme.stroke.width.thick : theme.stroke.width.thin });
            break;
        }
        case "circle": {
            const radius = size / 2;
            ...
            for (let i = 0; i < totalParts; i++) {
                ...
                canvas.drawPath(path, { fill: i < shadedParts ? shadeColor : "none", stroke: theme.colors.black, strokeWidth: theme.stroke.width.base });
            }
            break;
        }
        case "polygon": {
            ...
            for (let i = 0; i < totalParts; i++) {
                ...
                canvas.drawPath(path, { fill: isShaded ? shadeColor : "none", stroke: "none" });
            }
...
export const generateQuantityFractionalDiagram: WidgetGenerator<typeof QuantityFractionalDiagramPropsSchema> = async (props) => {
    const { width, height, shape, numerator, denominator, shadeColor, shapesPerRow } = props;

    const canvas = new CanvasImpl({ chartArea: { left: 0, top: 0, width, height }, fontPxDefault: 16, lineHeightDefault: 1.2 });
