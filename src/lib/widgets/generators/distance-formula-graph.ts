...
export const DistanceFormulaGraphPropsSchema = z
	.object({
		...
		width: z
			.number()
			.positive()
			.describe("Total width of the coordinate plane in pixels (e.g., 500, 600, 400). Should accommodate axis labels and distance annotations."),
		height: z
			.number()
			.positive()
			.describe("Total height of the coordinate plane in pixels (e.g., 500, 600, 400). Typically equal to width for square aspect ratio."),
		...
		showQuadrantLabels: z
			.boolean()
			.describe("Whether to show Roman numerals (I, II, III, IV) in quadrants. True helps with quadrant identification in distance problems."),
		points: z
			.array(createPlotPointSchema())
			.describe("Points to plot on the plane. Each point can have a label. Points referenced in distances should be defined here. Empty array if only showing distances."),
	...
	.strict()
	.describe("Creates a coordinate plane specifically designed for distance formula visualization. Shows the right triangle formed by two points with labeled legs (Δx, Δy) and hypotenuse (distance). Essential for teaching d = √[(x₂-x₁)² + (y₂-y₁)²] geometrically.")
