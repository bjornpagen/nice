...
export const AdditionWithRegroupingPropsSchema = z
	.object({
		...
		showAnswer: z
			.boolean()
			.describe(
				"Whether to show the complete solution with carrying marks and the sum. When true, displays carried values and the final answer. When false, shows only the problem setup."
			)
	})
	.strict()
	.describe(
...
export const generateAdditionWithRegrouping: WidgetGenerator<typeof AdditionWithRegroupingPropsSchema> = async (
	...
	const { addend1, addend2, showAnswer } = data
