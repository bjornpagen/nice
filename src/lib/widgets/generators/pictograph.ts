...
const Key = z.object({ 
  ...
  label: z.string().nullable().transform((val) => (val === "null" || val === "NULL" || val === "" ? null : val)).describe("What each icon represents with its value (e.g., '= 10 apples', '= 5 cars', '= 100 votes', null). Shows the scale/multiplier. Null means no label.") 
}).strict()
...
export const PictographPropsSchema = z.object({
  ...
  title: z.string().nullable().transform((val) => (val === "null" || val === "NULL" || val === "" ? null : val)).describe("Title displayed above the pictograph (e.g., 'Fruit Sales This Week', 'Favorite Pets', null). Null means no title."),
