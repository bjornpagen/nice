...
export const generatePolyhedronDiagram: WidgetGenerator<typeof PolyhedronDiagramPropsSchema> = async (data) => {
	...
	switch (shape.type) {
		case "rectangularPrism": {
			...
			const hidden =
				'stroke="${theme.colors.black}" stroke-width="${theme.stroke.width.base}" stroke-dasharray="${theme.stroke.dasharray.dashedShort}"'
		...
		case "triangularPyramid": {
			...
			for (const lab of labels) {
				if (lab.target === "baseLength" && p[0] && p[1]) {
					...
					canvas.drawText({ x: textX, y: textY, text: lab.text, anchor: "start", dominantBaseline: "middle" })
