...
function createShapeItemSchema() {
    return z.object({
        ...
        shape: z.enum(["triangle", "square", "rectangle", "pentagon", "hexagon", "heptagon", "octagon", "circle"])
                 .describe("The specific type of polygon or shape to render."),
        fillColor: z.string().regex(CSS_COLOR_PATTERN, "invalid css color")
...
export const PatternDiagramPropsSchema = z.object({
    type: z.literal("patternDiagram").describe("Identifies this as a widget for displaying a sequence of shapes to form a pattern."),
    ...
    shapeSize: z.number().positive().describe("The size (width and height) in pixels for each individual shape or placeholder in the sequence."),
}).strict();
