...
export const generateConceptualGraph: WidgetGenerator<typeof ConceptualGraphPropsSchema> = (props) => {
	...
	svgBody += `<line x1="${yAxisX}" y1="${xAxisY}" x2="${yAxisX}" y2="${margin.top}" stroke="${theme.colors.axis}" stroke-width="${theme.stroke.width.thick}" marker-end="url(#graph-arrow)"/>`
	svgBody += `<line x1="${yAxisX}" y1="${xAxisY}" x2="${width - margin.right}" y2="${xAxisY}" stroke="${theme.colors.axis}" stroke-width="${theme.stroke.width.thick}" marker-end="url(#graph-arrow)"/>`
	// --- ADDED ---
	...
	svgBody += `<polyline points="${pointsStr}" fill="none" stroke="${curveColor}" stroke-width="${theme.stroke.width.xxthick}" stroke-linejoin="round" stroke-linecap="round"/>`
	// --- ADDED ---
	...
	for (const hp of highlightPoints) {
		...
		svgBody += `<text x="${cx - highlightPointRadius - 5}" y="${cy}" text-anchor="end" dominant-baseline="middle" font-weight="${theme.font.weight.bold}">${hp.label}</text>`
