...
export function approxTextWidth(text: string, avgCharWidthPx: number = 7): number {
	return Math.max(0, text.length * avgCharWidthPx)
}

export function includeText(ext: Extents, absoluteX: number, text: string, anchor: "start" | "middle" | "end", avgCharWidthPx: number = 7): void {
...
export function computeDynamicWidth(ext: Extents, height: number, pad: number = 10): { vbMinX: number; vbMaxX: number; dynamicWidth: number } {
