...
export function computeAndRenderXAxis(
	...
	if (!isCategorical) {
		...
		if (!spec.categories || spec.categories.length === 0) {
			logger.error("invalid x-axis categories", { categories: spec.categories })
			throw errors.wrap(ErrInvalidCategories, "x-axis categories array cannot be empty")
		}
	}
	...
	function toSvgX(val: number): number {
		if (isCategorical) {
			const bw = chartArea.width / (spec.categories as string[]).length
	...
	let bandWidth: number | undefined = undefined
	if (isCategorical) {
		bandWidth = chartArea.width / (spec.categories as string[]).length
	}
	...
	if (isCategorical) {
		const cats = spec.categories as string[]
		for (let i = 0; i < cats.length; i++) {
			tickPositions.push(toSvgX(i))
			tickLabels.push(cats[i] as string)
		}
