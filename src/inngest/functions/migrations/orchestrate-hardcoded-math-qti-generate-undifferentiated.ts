...
export const orchestrateHardcodedMathQtiGenerateUndifferentiated = inngest.createFunction(
	{
		...
		name: "Orchestrate Hardcoded Math Course Undifferentiated QTI Generation"
	},
	{ event: "migration/hardcoded.math.qti.generate-undifferentiated" },
	async ({ logger }) => {
		logger.info("starting hardcoded math undifferentiated qti generation", {
		...
		await Promise.all(
			courses.map(async (course) => {
				...
				logger.info("processing math course for undifferentiated generation", { courseId })
...
<qti-assessment-test xmlns=\"http://www.imsglobal.org/xsd/imsqtiasi_v3p0\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xsi:schemaLocation=\"http://www.imsglobal.org/xsd/imsqtiasi_v3p0 https://purl.imsglobal.org/spec/qti/v3p0/schema/xsd/imsqti_asiv3p0_v1p0.xsd\" identifier=\"nice_${exercise.id}\" title=\"${safeTitle}\">
    ...
    <qti-test-part identifier=\"PART_1\" navigation-mode=\"nonlinear\" submission-mode=\"individual\">\n${selectionStrategy}\n    </qti-test-part>\n</qti-assessment-test>`
				...
				logger.info("generated undifferentiated items, stimuli, and tests for math course", {
					courseId,
					items: items.length,
		...
		logger.info("completed hardcoded math undifferentiated qti generation for all courses")
