...
export const ingestUndifferentiatedCourseFromDb = inngest.createFunction(
	{
		id: "ingest-undifferentiated-course-from-db",
		name: "Ingest Undifferentiated Course from DB"
	},
	{ event: "qti/course.ingest.undifferentiated" },
	async ({ event, step, logger }) => {
		const { courseId } = event.data
		logger.info("starting undifferentiated qti json dump from db", { courseId })

		// Step 1: Fetch all course structure and content IDs from the database.
		// This part is NOT in step.run() because it's a series of database queries.
		...
		const courseSlug = courseResult.data.slug
		...
		// Step 2: Assemble the JSON payloads from the fetched data.
...
<qti-assessment-test xmlns="http://www.imsglobal.org/xsd/imsqtiasi_v3p0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.imsglobal.org/xsd/imsqtiasi_v3p0 https://purl.imsglobal.org/spec/qti/v3p0/schema/xsd/imsqti_asiv3p0_v1p0.xsd" identifier="nice_${exercise.id}" title="${safeTitle}">
	<qti-test-part identifier="PART_1" navigation-mode="nonlinear" submission-mode="individual">
		...
		// Step 3: Write the final JSON files to the data/ directory inside a step.
		const outputDir = await step.run("write-qti-json-files", async () => {
			...
			const writeResults = await errors.try(Promise.all(writePromises))
			if (writeResults.error) {
				logger.error("failed to write one or more qti json files", { error: writeResults.error })
				throw errors.wrap(writeResults.error, "file write operation")
			}

			return courseDir
		})

		logger.info("completed undifferentiated qti json dump", {
			courseId,
			outputDir,
			stats: {
				items: items.length,
		...
		return {
			message: "Undifferentiated QTI JSON dump workflow completed successfully.",
			courseId,
			outputDir,
			skippedItems: skippedItemsCount
		}
